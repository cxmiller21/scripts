    ...
    try:
        ...
        for user in users:
            ...
            if len(user_access_keys['AccessKeyMetadata']) > 0:
                for access_key in user_access_keys['AccessKeyMetadata']:
                    ...
                    if 'Inactive' in status:
                        print(f'AK was {access_key["Status"]}: Deleting {user_name}\'s old access key with ID {access_key_id}')
                        client.delete_access_key(UserName=user_name, AccessKeyId=access_key_id)
                        create_new_access_key = True
                    if 'Active' in status:
                        print(f'AK was {access_key["Status"]}: Inactivating {user_name}\'s primary access key with ID {access_key_id}')
                        client.update_access_key(UserName=user_name, AccessKeyId=access_key_id, Status='Inactive')
                        create_new_access_key = True
            if create_new_access_key:
                ...
                user_access_key = build_user_access_key_dict(client_access_key['AccessKey'])
...
def rotate_access_keys():
    ...
    if len(users):
        ...
        print(f'Created {len(new_access_keys)} new user access keys.\nGenerating the user_access_keys.csv file')
